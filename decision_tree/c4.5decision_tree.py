# -*- coding: utf-8 -*-
# @Time    : 2019/1/3 上午11:53
# @Author  : xuef
# @FileName: c4.5decision_tree.py
# @Software: PyCharm
...
import operator
from decision_tree.test import treePlotter
...
def train_test_split(X, test_size = 0.2, shuffle = True, seed=None):
    if shuffle:
        X= shuffle_data(X, seed)
...
if __name__ == '__main__':
    ...
    # np.insert(dataSet, 4, values=target, axis=1)
    dataSets = np.column_stack((data, target))
    print(iris.data.shape)
    print(dataSets)
    data_train, data_test= train_test_split(dataSets, test_size=0.33, shuffle=True)
    # labels = ['萼片长度', '萼片宽度', '花瓣长度', '花瓣宽度']
    ...
    print("---------------------------------------------")
    print(u"数据集长度", len(data_train))
    print("Ent(D):", jisuanEnt(data_train))
    print("---------------------------------------------")

    print(u"以下为首次寻找最优索引:\n")
    print(u"ID3算法的最优特征索引为:" + str(ID3_chooseBestFeatureToSplit(data_train.tolist())))
    print("--------------------------------------------------")
    print(u"C4.5算法的最优特征索引为:" + str(C45_chooseBestFeatureToSplit(data_train.tolist())))
    print("--------------------------------------------------")
    print(u"CART算法的最优特征索引为:" + str(CART_chooseBestFeatureToSplit(data_train.tolist())))
    print(u"首次寻找最优索引结束！")
    print("---------------------------------------------")
    ...
    while (True):
        ...
        if dec_tree == '1':
            ...
            ID3desicionTree = ID3_createTree(data_train.tolist(), labels_tmp)
            ...
            accuracy1 = accuracy(result, testSet)
            # accuracy = accuracy(result, testSet)
            print("下面为测试数据集结果：")
        ...
        if dec_tree == '3':
            labels_tmp = labels[:]  # 拷贝，createTree会改变labels
            CARTdesicionTree = CART_createTree(data_train.tolist(), labels_tmp)
            print('CARTdesicionTree:\n', CARTdesicionTree)
            treePlotter.CART_Tree(CARTdesicionTree)
            testSet = data_test
            result3=classifytest(CARTdesicionTree, labels, testSet)
            accuracy3 = accuracy(result3, testSet)
            print("下面为测试数据集结果：")
