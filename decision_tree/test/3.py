...
from decision_tree.test import plotter
...
def load_data(train_size):
    ...
    train_data, test_data = train_test_split(data_set, test_size=0.33)  # 测试集所占的比例
...
if __name__ == '__main__':
    ...
    # dataSets[-1].astype(int)
    # print(dataSets[-1].astype(int))

    # data_train, data_test = load_data(dataSets)
    data_train, data_test = train_test_split(dataSets, test_size=0.33)
    ...
    decision_tree = generate_decision_tree(data_train, labels)
    # 递归会改变attribute_label的值，此处再传一次

    labels = ['萼片长度', '萼片宽度', '花瓣长度', '花瓣宽度']
    count = 0
    # 计算准确率
    for one_test_data in data_test:
        if decision_tree_predict(decision_tree, labels, one_test_data) == one_test_data[-1]:
            count += 1
    accuracy = count / len(data_test)
    print('训练集大小%d，测试集大小%d，准确率为:%.1f%%' % (data_train, len(data_test), 100 * accuracy))
    plotter.createPlot(decision_tree)
