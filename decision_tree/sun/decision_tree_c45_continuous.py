...
def calcInfoGainForSeries(dataSet, i, baseEntropy):
    ...
    for mid in midFeatList:
        ...
        # 计算两部分的特征值熵和权重的乘积之和
        # print(len(eltDataSet), len(gtDataSet), len(featList))
        p = len(eltDataSet)/len(featList)
        newEntropy = p*calcShannonEnt(eltDataSet) + (1-p)*calcShannonEnt(gtDataSet)
        # 计算出信息增益
        infoGain = baseEntropy - newEntropy
        # print(p, baseEntropy, infoGain)
        # print('当前划分值为：' + str(mid) + '，此时的信息增益为：' + str(infoGain))
        if infoGain > maxInfoGain:
            bestMid = mid
            maxInfoGain = infoGain
            splitinfo = -p*math.log(p, 2) - (1-p)*math.log(1-p, 2)
    # print(maxInfoGain, bestMid, splitinfo)
    return maxInfoGain, bestMid, splitinfo
...
def chooseBestFeatureToSplit(dataSet, labels):
    ...
    for i in range(numFeatures):
        ...
        # 得到数据集中所有的当前特征值列表
        featList = [example[i] for example in dataSet]
        # print('isinstance(featList[0], str):', isinstance(featList[0], str))
        if isinstance(featList[0], str):
            infoGain, splitinfo = calcInfoGain(dataSet, featList, i, baseEntropy)
            featEntroy[i] = [infoGain, infoGain/splitinfo]
        else:
            # print('当前划分属性为：' + str(labels[i]))
            infoGain, bestMid, splitinfo = calcInfoGainForSeries(dataSet, i, baseEntropy)
            print(infoGain, bestMid, splitinfo)
            if splitinfo == 0: continue
            featEntroy[i] = [infoGain, infoGain/splitinfo, bestMid]
            # print(infoGain, bestMid)

        # print('当前特征值为：' + labels[i] + '，对应的信息增益值为：' + str(infoGain))

    meanEntropy = 0
    for key, value in featEntroy.items():
        meanEntropy += value[0]
    meanEntropy /= len(featEntroy)
    for key, value in featEntroy.items():
        if value[0] >= meanEntropy and bestInfoGain < value[1]:
            bestInfoGain = value[1]
            bestFeature = key

    if not isinstance(dataSet[0][bestFeature], str):
        flagSeries = 1
        bestSeriesMid = featEntroy[bestFeature][2]
...
def classify(inputTree, featLabels, testVec):
    ...
    classLabel = -1
    if '小于' in firstStr:
        # str(labels[bestFeat[0]]) + '小于' + str(bestFeat[1]) + '?'
        feature= re.findall('(.*?)小于(.*?)\?', firstStr)
        # print(feature, type(feature), type(feature[0]))

        # print(firstStr, featLabels)
        featIndex = featLabels.index(feature[0][0])

        if testVec[featIndex] > float(feature[0][1]):
...
def train_test_split(data, test_size = 0.2, shuffle = True, seed=None):
    ...
    n_train_samples = int(data.shape[0]*(1-test_size))
    x_train, x_test = data[:n_train_samples], data[n_train_samples:]

    return x_train, x_test


def predict(myTree, featLabels, X_test):
    y_pred = []
    for sample in X_test:
        # print(sample)
        label= classify(myTree, featLabels, sample)
        y_pred.append(label)
